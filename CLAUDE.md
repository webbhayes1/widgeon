...
- Phase 3 DONE (uncommitted): pixel sprite engine. `Shared/Sprites.swift` (generated from handoff HTML — 39 16×16 sprites + 32-color palette; do not hand-edit, regenerate). `PixelSpriteView` renders a sprite grid via SwiftUI Canvas (crisp integer-scaled pixels, `.color(hex:)`). `PetStage` gained a `sprite` key; each character maps 8 XP stages to sprites (duck: egg→hatch→duckling→duckB/T/A→noble→royal; others use their tamagotchi baby + B/T/A illustrated + tamagotchi adult, repeating the top frame where sprites run out). Wired into PetView (onboarding egg, character picker, 176px home sprite) + the pet widget's home-screen (color) case. Accessory/lock widgets keep emoji (system-tinted). Emoji retained on `PetStage` as accessibility labels/fallback.
